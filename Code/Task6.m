%% Task 6: 4th scan - three targets

clc;
clear;
close all

%% Part a: Target-1,2,3 parameters are known: generate synthetic backscatter data for this scan
...
%% Part b: Plot the backscatter-data for the dwell time that corresponds to the direction of the three targets
...
if targets == 3
    theta_steer_angle3 = 120;
    j3 = theta_steer_angle3 - 29;
    z3 = z_out(j3,:);
end
...
% Steering direction = 120 degrees
figure();
semilogy(abs(z3));
title('Signal at point Z due to Target-3');
xlabel('Time Index (Tc)');
ylabel('Magnitude (V)');
xlim([0 11200]);
ylim([1e-7 1.5e-3]);
...
% hold on;
% semilogy(abs(z3));
% title('Signal at point Z due to Target-1, Target-2 and Target-3');
% xlabel('Time Index (Tc)');
% ylabel('Magnitude (V)');
% xlim([0 11200]);
% ylim([1e-7 1e-3]);
...
%     xline(1400*i, 'red', ['PRI = ', num2str(i)], 'LineWidth', 1,'LabelHorizontalAlignment', 'Left');
% end
% legend('Target-1' , 'Target-2', 'Target-3', 'Location', 'Northeast');
% set(gca, 'Fontsize', 14);

%% Part c: Using only your generated backscatter random numbers detect and estimate the parameters of the three target.
...
%% Setting the parameters for the target according to the direction found
...
%% Define the Rx correlatoras: as a Matched Filter function
